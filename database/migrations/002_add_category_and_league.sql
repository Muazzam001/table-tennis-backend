-- Migration 002: Player category and league columns on teams/matches
-- Usage: mysql -u root -p table_tennis_tournament < database/migrations/002_add_category_and_league.sql
...
SET @sql = IF(@col_exists = 0,
  "ALTER TABLE teams ADD COLUMN league ENUM('Expert', 'Intermediate', 'Women') NOT NULL DEFAULT 'Expert' AFTER player2_id",
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0,
  "ALTER TABLE matches ADD COLUMN league ENUM('Expert', 'Intermediate', 'Women') NOT NULL DEFAULT 'Expert' AFTER pool",
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
